namespace Alter.Runtime.Character
{
    ...
    public class CharacterRagdoll
    {
        ...
        [SerializeField] private StartRagdollOn autoRagdollOn = StartRagdollOn.None;
        public StartRagdollOn AutoRagdollOn => autoRagdollOn;
        [SerializeField] private float minVelocityRagdoll = -12f;

        public bool isBusy = false;

        // INITIALIZERS: --------------------------------------------------------------------------

        public CharacterRagdoll(ICharacter character)
        {
            this.character = character;
            this.charAnimation = this.character.CharacterAnimation;
            this.standFaceDown = character.Ragdoll.standFaceDown;
            this.standFaceUp = character.Ragdoll.standFaceUp;
            this.ragdollMass = character.Ragdoll.ragdollMass;
            this.autoRagdollOn = character.Ragdoll.autoRagdollOn;
            this.minVelocityRagdoll = character.Ragdoll.minVelocityRagdoll;

            if (this.autoRagdollOn == StartRagdollOn.Land)
                this.character.EventOnCharacterLand += OnCharacterLand;
            this.character.EventAfterChangeModel += ReInitializeRagdoll;
            this.Initialize(true);
        }
        void ReInitializeRagdoll()
        {
            isInitialized = false;
            this.charAnimation = this.character.CharacterAnimation;
            this.Initialize(true);
        ...
        public void Initialize(bool forceInitialize = false)
        {
            ...
            if (this.charAnimation == null || !this.charAnimation.animator.isHuman) return;
        ...
        public void Update()
        {
            if (state == State.Normal && character.CharacterDriver.WorldMoveDirection.y <= minVelocityRagdoll)
            {
                character.SetRagdoll(true, true);
                return;
            }
            switch (this.state)
            {
        ...
        private void UpdateRecover()
        {
            ...
            if (t >= 1f)
            {
                ...
                if (StandupTask == null)
                {
                    this.character.CharacterDriver.SetActiveController(true);
                }
        ...
        // PRIVATE METHODS: -----------------------------------------------------------------------
        private void OnCharacterLand(float velocity)
        {
            if (velocity <= this.minVelocityRagdoll)
                this.character.SetRagdoll(true, true);
        }
        private void ToRagdoll()
        {
            if (this.character.IsRagdoll()) return;
            this.character.Gestures.Stop(0, 0);
            //this.charAnimation.StopGesture();
            ...
            Vector3 velocity = this.character.CharacterDriver.WorldMoveDirection;//.characterController.velocity;
            for (int i = 0; i < this.bones.Length; ++i)
            {
        ...
        private async void ToRecover()
        {
            ...
            for (int i = 0; i < hitCount; ++i)
            {
                ...
                {
                    float offset = this.character.CharacterDriver.SkinWidth;
            ...
            gestureConfig.TransitionIn = 0f;
            gestureConfig.TransitionOut = 0.25f;
            gestureConfig.Speed = 1f;

            switch (this.root.anchor.forward.y < 0f)
            {
                case true:
                    if (this.standFaceDown)
                    {
                        gestureConfig.Duration = this.standFaceDown.length;
                        StandupTask = this.character.Gestures.CrossFade(this.standFaceDown, null, BlendMode.Blend, gestureConfig, true);
                    }
                ...
                case false:
                    if (this.standFaceUp)
                    {
                        gestureConfig.Duration = this.standFaceUp.length;
                        StandupTask = this.character.Gestures.CrossFade(this.standFaceUp, null, BlendMode.Blend, gestureConfig, true);
                    }
        ...
        {
            Animator animator = this.charAnimation.animator;
        ...
        private bool BuildMasses()
        {
            for (int i = 0; i < this.bones.Length; ++i)
            {
                this.bones[i].rigidbody.mass *= this.ragdollMass;
            }
        ...
        {
            Transform target = this.charAnimation.animator.transform;
